    ...
    if len(message.command) > 1:
        ...
        if payload.startswith("verify_"):
            ...
            if verify_status.get('verify_token') != token:
                return await message.reply("𝖫𝗂𝗇𝗄 𝖤𝗑𝗉𝗂𝗋𝖾𝖽 𝗈𝗋 𝖨𝗇𝗏𝖺𝗅𝗂𝖽. 𝖯𝗅𝖾𝖺𝗌𝖾 𝗋𝖾𝗊𝗎𝖾𝗌𝗍 𝗍𝗁𝖾 𝖿𝗂𝗅𝖾 𝖺𝗀𝖺𝗂𝗇.")
            ...
            return await message.reply(
                f"𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝗏𝖾𝗋𝗂𝖿𝗂𝖾𝖽. 𝖸𝗈𝗎𝗋 𝖺𝖼𝖼𝖾𝗌𝗌 𝗂𝗌 𝗏𝖺𝗅𝗂𝖽 𝖿𝗈𝗋 𝗍𝗁𝖾 𝗇𝖾𝗑𝗍 {client.verify_expire} 𝗌𝖾𝖼𝗈𝗇𝖽𝗌.",
        ...
        if is_still_verified:
            ...
            btn = [
                ...
                [InlineKeyboardButton("𝖡𝗎𝗒 𝖯𝖺𝗂𝖽", url="https://t.me/Rtx_Contect_bot")]
            ]
            verify_photo = client.messages.get("VERIFY_PHOTO", "")
            
            caption = f"""𝖵𝖾𝗋𝗂𝖿𝗂𝖼𝖺𝗍𝗂𝗈𝗇 𝖱𝖾𝗊𝗎𝗂𝗋𝖾𝖽

𝖸𝗈𝗎 𝖠𝗋𝖾 𝖭𝗈𝗍 𝖵𝖾𝗋𝗂𝖿𝗂𝖾𝖽
...
- 𝖯𝗋𝗂𝗈𝗋𝗂𝗍𝗒 𝗌𝗎𝗉𝗉𝗈𝗋𝗍"""
        ...
        buttons = [
            [InlineKeyboardButton("𝖠𝖻𝗈𝗎𝗍", callback_data="about"), InlineKeyboardButton("𝖢𝗅𝗈𝗌𝖾", callback_data='close')]
        ]
...
async def get_user_state_with_cache(client: Client, user_id: int):
    now = time.time()
    state, paid_expires_at = await client.mongodb.get_user_state(user_id)
    if state is not None:
        client.user_cache[user_id] = {'state': state, 'timestamp': now, 'paid_expires_at': paid_expires_at}
    return state
...
async def request_command(client: Client, message: Message):
    ...
    if user_state is None or not user_state.get('is_paid', False): 
        return await message.reply(
            "𝖮𝗇𝗅𝗒 𝗉𝖺𝗂𝖽 𝗎𝗌𝖾𝗋𝗌 𝖼𝖺𝗇 𝗆𝖺𝗄𝖾 𝗋𝖾𝗊𝗎𝖾𝗌𝗍𝗌.", 
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("𝖡𝗎𝗒 𝖯𝖺𝗂𝖽", url="https://t.me/Rtx_Contect_bot")]
            ])
        )
    ...
    if len(message.command) < 2: 
        return await message.reply("𝖴𝗌𝖺𝗀𝖾: /request <𝖼𝗈𝗇𝗍𝖾𝗇𝗍 𝗇𝖺𝗆𝖾>")
...
𝖳𝗁𝖺𝗇𝗄 𝗒𝗈𝗎 𝖿𝗈𝗋 𝖻𝖾𝗂𝗇𝗀 𝖺 𝗉𝖺𝗂𝖽 𝗎𝗌𝖾𝗋!"""
    else:
        plan_text = f"""𝖸𝗈𝗎𝗋 𝖯𝗋𝗈𝖿𝗂𝗅𝖾
